...
text = replace_once(text, '''function compareCurrentOrder(a,b,now=Date.now()){
...
text = replace_once(text, '''  function releasedEpisodeCounts(item){
...
text = replace_once(text, '      const episodes=[...new Set(source.map(Number).filter(episode=>Number.isInteger(episode)&&episode>0&&(!maximum||episode<=maximum)))].sort((a,b)=>a-b);', '      const episodes=[...new Set(source.map(Number).filter(episode=>Number.isInteger(episode)&&episode>0&&maximum>0&&episode<=maximum))].sort((a,b)=>a-b);', "watched episode normalization")
text = replace_once(text, '    else if(activeSort==="recent")list.sort((a,b)=>addedSortTime(b)-addedSortTime(a)||titleCollator.compare(a.title||"",b.title||""));', '    else if(activeSort==="recent")list.sort((a,b)=>compareRecentOrder(a,b,now));', "recent sort")
index_path.write_text(text)

reminder_path = Path("supabase/functions/watchlog-reminders/index.ts")
reminder = reminder_path.read_text()
reminder = replace_once(reminder, '''  const title = `Watched Logger reminder for ${name}`;
  let body = `Your Planned show is due ${timing}.`;
  if (event.kind === "episode" && event.number) {
    body = leadHours === 0
      ? `Episode ${event.number} is available now.`
      : `Episode ${event.number} airs ${timing}.`;''', '''  const title = "Watched log reminder";
  let body = `${name} is due ${timing}.`;
  if (event.kind === "episode" && event.number) {
    body = leadHours === 0
      ? `${name} episode ${event.number} is out now.`
      : `${name} episode ${event.number} airs ${timing}.`;''', "reminder copy")
reminder = replace_once(reminder, '''        title: `Watched Logger reminder for ${String(planned.title || "Planned title")}`,
        body:
          "Test reminder — background alerts work when Watched Logger is closed.",''', '''        title: "Watched log reminder",
        body: `${String(planned.title || "Planned title")} test reminder is out now.`,''', "private test copy")
